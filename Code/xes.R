...
getSymbols("XES", from = "2026-02-28", to = "2026-03-09", auto.assign = T)

last_price <- last(as_tibble(XES, rownames = "Date"))$XES.Close
old_price <- first(as_tibble(XES, rownames = "Date"))$XES.Close
...
state_space <- seq(65, 165, by = 10)

iter <- 5000
chains <- 4

dataset_new <-
  read_csv("data/XES Options.csv") |>
  filter(date == "2026-03-09")
...
saveRDS(results_xes, file = "Data/Results/results_xes.RData")
saveRDS(results_xes_old, file = "Data/Results/results_xes_old.RData")
results_xes <- readRDS("Data/Results/results_xes.RData")
results_xes_old <- readRDS("Data/Results/results_xes_old.RData")
...
betas_compare <-
  results_xes_old[[2]]$betas |>
  mutate(date = "2026-02-26") |>
  bind_rows(results_xes[[2]]$betas |> mutate(date = "2026-03-09"))
...
betas_plot <-
  ...
  geom_errorbar(aes(max = conf.high.hpd.0.95, min = conf.low.hpd.0.95), width = .01, alpha = 0.5, size = 0.5) +
...
ggsave("~/Library/CloudStorage/OneDrive-Personal/MACRO/export/oil_serv_S&P_implied_probability_density.png", betas_plot,
  width = 9 / 1.6, height = 9 / 1.6
)

# 
# results[[2]]$coefs |> filter(term == "alpha")
# 
# results_old[[2]]$coefs |> filter(term == "alpha")
...
betas_plot_1 <-
  ...
  labs(title = "Имплицитое распределение \nS&P Oil & Gas Equipment & Services\nк 2026-04-17", caption = "95% highest posterior density interval") +
  theme_light() +
  theme(
    plot.title = element_text(size = 11, color = "#2F2926"),
    text = element_text(size = 12, color = "#2F2926", family = "SB Sans Interface"),
...
ggsave("~/Library/CloudStorage/OneDrive-Personal/MACRO/export/oil_serv_S&P_implied_probability_density_combined.png", betas_plot_1,
       width = 9 / 1.6, height = 9 / 1.6
)
